...
public class PatientSAQ7_1Controller {
   @FXML private Button naeste4;
   @FXML private Button tilbage5;
   @FXML private Button logAf5;
   
   
   @FXML private void handleButtonActionNaeste4 (ActionEvent e) {
        try{  //try bruges til at fange exceptions, og exceptions er hvis der opstår noget fejl
            URL fxmlURL = getClass().getResource("PatientSAQ7_2.fxml"); //sætter URL til at være en sti til PatientSAQ7_2.fxml (sti bruges til at betegne hvor man klikker hen)
            Parent tableViewParent = FXMLLoader.load(fxmlURL);  //loader URL (stien) og herved loader PatientSAQ7_2.fxml. Parent indikerer at det er en ny scene. 
            Scene tableViewScene = new Scene(tableViewParent); //Laver ny scene med PatientSAQ7_2 her. tableViewScene er variabelnavnet til en ny scene

            //herfra sættes stagen nu hvor man har klikket på næste-knappen:
            Stage window = (Stage) naeste4.getScene().getWindow(); //laver en ny stage som hedder window
      ...
      @FXML private void handleButtonActionTilbage5 (ActionEvent e) {
        try{  //try bruges til at fange exceptions, og exceptions er hvis der opstår noget fejl
            ...
            Stage window = (Stage) tilbage5.getScene().getWindow(); //laver en ny stage som hedder window
            window.setScene(tableViewScene); //sætter scenen for window med den scene fra l.23
            window.show(); //koden her fremviser det nye stage 
        }catch (IOException io){ //catch fanger en IOException (IO fejl) i dette tilfælde 
            io.printStackTrace(); //hvis der er en IOException så printer den "StackTrace" og hvad stacken indeholder. Debugging.
        ...
        try{  //try bruges til at fange exceptions, og exceptions er hvis der opstår noget fejl
            ...
            //herfra sættes stagen nu hvor man har klikket  på login-knappen:
            Stage window = (Stage) logAf5.getScene().getWindow(); //laver en ny stage som hedder window
            window.setScene(tableViewScene); //sætter scenen for window med den scene fra l.23
            window.show(); //koden her fremviser det nye stage 
        }catch (IOException io){ //catch fanger en IOException (IO fejl) i dette tilfælde 
            io.printStackTrace(); //hvis der er en IOException så printer den "StackTrace" og hvad stacken indeholder. Debugging.
